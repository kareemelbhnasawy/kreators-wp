 ...
 */
{
    ...
    class Slide {
        constructor(el, title) {
            ...
            this.DOM.number = this.DOM.el.querySelector('.mt-addons-crossroads-slider-number');
            this.DOM.subtitle = this.DOM.el.querySelector('.mt-addons-crossroads-slider-caption');
            this.DOM.imgWrap = this.DOM.el.querySelector('.mt-addons-crossroads-slider-img-wrap');
            this.DOM.img = this.DOM.imgWrap.querySelector('.mt-addons-crossroads-slider-img');
        }
        move(direction, val) {
        ...
        setCenter() {
            this.isCenter = true;
            this.DOM.el.classList.add('mt-addons-crossroads-slider-grid__item--center');
            this.DOM.title.classList.add('mt-addons-crossroads-slider-grid__item--center');
            TweenMax.set([this.DOM.el, this.DOM.title], {opacity: 1});
        }
        setRight() {
            this.isRight = this.isCenter = false;
            this.isLeft = true;
            this.DOM.el.classList.add('mt-addons-crossroads-slider-grid__item--right');
            this.DOM.title.classList.add('mt-addons-crossroads-slider-grid__item--right');
            TweenMax.set([this.DOM.el, this.DOM.title], {opacity: 1});
        }
        setLeft() {
            this.isLeft = this.isCenter = false;
            this.isRight = true;
            this.DOM.el.classList.add('mt-addons-crossroads-slider-grid__item--left');
            this.DOM.title.classList.add('mt-addons-crossroads-slider-grid__item--left');
            TweenMax.set([this.DOM.el, this.DOM.title], {opacity: 1});
        }
        reset() {
            ...
            this.DOM.title.classList = 'mt-addons-crossroads-slider-grid__item mt-addons-crossroads-slider-grid__item--title';
            this.DOM.el.classList = 'mt-addons-crossroads-slider-grid__item mt-addons-crossroads-slider-grid__item--slide';
        }
        animateElementsOut(contentItem) {
            return new Promise((resolve, reject) => {
                ...
                if ( this.isCenter ) {
                    const contentItemTitle = contentItem.querySelector('.mt-addons-crossroads-slider-content__item-header-title');
                    const contentItemImg = contentItem.querySelector('.mt-addons-crossroads-slider-img--content');
    ...
    class Slideshow {
        constructor(el) {
            this.DOM = {el: el};
            // The titles
            this.DOM.titlesWrap = this.DOM.el.querySelector('.mt-addons-crossroads-slider-titles-wrap');
            this.DOM.titlesInner = this.DOM.titlesWrap.querySelector('.mt-addons-crossroads-slider-grid--titles');
            this.DOM.titles = [...this.DOM.titlesInner.querySelectorAll('.mt-addons-crossroads-slider-grid__item--title')];
            // The slides instances
            this.slides = [];
            [...this.DOM.el.querySelectorAll('.mt-addons-crossroads-slider-grid__item--slide')].forEach((slide, pos) => this.slides.push(new Slide(slide, this.DOM.titles[pos])));
            ...
            // Content Items
            this.DOM.contentItems = [...document.querySelectorAll('.mt-addons-crossroads-slider-content__item')];

            // Areas (left, center, right) where to attach the navigation events.
            this.DOM.interaction = {
                left: document.querySelector('.mt-addons-crossroads-slider-grid__item--left'),
                center: document.querySelector('.mt-addons-crossroads-slider-grid__item--center'),
                right: document.querySelector('.mt-addons-crossroads-slider-grid__item--right')
            };
        ...
        initEvents() {
            ...
            this.DOM.contentItems.forEach(item => {
                item.querySelector('.mt-addons-crossroads-slider-img-wrap--content').addEventListener('click', () => this.closeSlide());
            });
        }
        navigate(direction) {
            if ( this.isAnimating ) {
        ...
        toggleSlide(action) {
            ...
            this.DOM.el.classList[action === 'open' ? 'add' : 'remove']('mt-addons-crossroads-slider-content-open');
            ...
            if ( action === 'open' ) {
                contentItem.classList.add('mt-addons-crossroads-slider-content__item--current');
            }
            Promise.all(promises).then(() => {
                if ( action === 'close' ) {
                    contentItem.classList.remove('mt-addons-crossroads-slider-content__item--current');
                }
                this.isAnimating = false;
            });
        }
    ...
    // Revealer element
    const revealer = new Revealer(document.querySelector('.mt-addons-crossroads-slider-revealer__inner'));
    // Initialize the slideshow
    new Slideshow(document.querySelector('.mt-addons-crossroads-slider-grid--slideshow'));

    // Preload all the images in the page
    imagesLoaded(document.querySelectorAll('.mt-addons-crossroads-slider-img'), {background: true}, () => document.body.classList.remove('mt-addons-crossroads-slider-loading'));
}